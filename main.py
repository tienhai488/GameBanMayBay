...
running = True
while running:
    player_bullet_fx.set_volume(volume_game)
    click_fx.set_volume(volume_game)
    collision_fx.set_volume(volume_game)
    blast_fx.set_volume(volume_game)
    fuel_fx.set_volume(volume_game)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if home_page:
                ...
                if p.rect.collidepoint((x, y)):
                    # khi nhan vao player thi ban dan 
                    shoot_bullet()
                elif x <= WIDTH // 2:
                    moving_left = True
                elif x > WIDTH // 2:
    ...
    if game_page:
        ...
        if delta_time >= plane_frequency:
            ...
            e = Enemy(x, -150, type,volume_game)
            enemy_group.add(e)
            start_time = current_time
            
            if type in (1, 2, 3):
                fx = pygame.mixer.Sound('Sounds/plane.mp3')
            elif type in (4, 5):
                fx = pygame.mixer.Sound('Sounds/chopper.mp3')
            fx.set_volume(volume_game)
            fx.play()
        ...
        p.fuel -= 0.05
        bg.update(1)
        # toc do duy chuyen cua ban do la 1
