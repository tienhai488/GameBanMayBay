...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y, type_,volume_game = 1):
        ...
        # self.fx.set_volume(volume_game)
        # self.fx.play()
...
class Bullet(pygame.sprite.Sprite):
    def __init__(self, x, y, type_, dx=None):
        # dx dung khi co powerup de dan bay ve hai phia
...
class Button(pygame.sprite.Sprite):
    ...
    def draw(self, win):
        ...
        # tri ve vi tri x y cua con tro chuot 
        if self.rect.collidepoint(pos):
            if pygame.mouse.get_pressed()[0] and not self.clicked:
                # tra ve cac trang thai cua chuot 
                action = True
                self.clicked = True
